...
apt-get update
apt-get install -y iputils-ping iputils-tracepath redis-tools mysql-client postgresql-client
curl -sSLf https://dl.k8s.io/v1.20.5/kubernetes-client-linux-amd64.tar.gz | tar -C /usr/local/bin -zx --strip-components=3
curl -sSLf https://releases.hashicorp.com/terraform/1.3.4/terraform_1.3.4_linux_amd64.zip | funzip >/usr/local/bin/terraform
chmod a+rx /usr/local/bin/terraform
curl -sSLf https://get.helm.sh/helm-v3.10.1-linux-amd64.tar.gz | tar -C /usr/local/bin -zx --strip-components=1 --exclude='LICENSE*' --exclude='README*' --no-same-owner
...
/tmp/aws/install
curl -sSLf https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-sdk-408.0.1-linux-x86_64.tar.gz | tar -C /usr/local -zx
